...
# --- INTEGRATED STEP: Drop genes with zero expression ---
...
# Filter: Keep rows where at least one value is NOT zero
df_temp = df_temp.loc[(df_temp != 0).any(axis=1)]

# Update variables for the rest of the pipeline
# Transpose back to (Cells x Genes) for X_all
...
for fold, (train_idx, test_idx) in enumerate(tqdm(skf.split(X_all, y_all), total=N_SPLITS, desc="CV Folds")):
    ...
    # --- Data Splitting (No validation set) ---
    ...
    # Process Test Data
    # IMPORTANT: We process test data independently to avoid data leakage
    ...
    # --- Pipeline Steps (Feature Selection, Scaling, PCA) ---
    # Ensure k is not larger than available features after filtering
    ...
    # --- Train Specialist Logistic Regression Models per Cluster ---
    best_model_objects = {}
    ...
    for cluster_id in sorted(np.unique(train_leiden_clusters)):
        ...
        # Skip if insufficient samples or only one class
        if X_train_c.shape[0] < 5 or len(np.unique(y_train_c)) < 2:
            continue

        # Calculate class weights for this cluster to handle imbalance
...
# --- 3. Aggregate, Report, and SAVE Final Results ---
print("\n\n#################################################")
...
# --- Save Detailed Fold Results CSV ---
...
annot_labels = [f"{val:.2f}\n(±{std:.2f})" for val, std in zip(mean_cm.flatten(), std_cm.flatten())]
annot_labels = np.asarray(annot_labels).reshape(mean_cm.shape)

plt.figure(figsize=(10, 8))
sns.heatmap(mean_cm, annot=annot_labels, fmt='', cmap='Blues',
            xticklabels=cm_labels, yticklabels=cm_labels, cbar=True)
plt.title('Mean Confusion Matrix (Test Set, Normalized by True Label)\nLogistic Regression with Balanced Class Weights', 
          fontsize=14, fontweight='bold')
plt.xlabel('Predicted Label', fontsize=12)
plt.ylabel('True Label', fontsize=12)
plt.tight_layout()

cm_plot_path = os.path.join(OUTPUT_DIR, "mean_confusion_matrix.png")
